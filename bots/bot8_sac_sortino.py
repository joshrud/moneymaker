"""
Bot 8 — SAC Portfolio with Sortino Reward
Same two-stage architecture as bot2 (StockSelector → SAC) but trained with Sortino
ratio as the reward function. Only penalises downside volatility — should be more
aggressive in up-markets and more defensive in drawdowns.
"""
...
class SortinoSACBot(BaseBot):
    def __init__(self, client: AlpacaClient, feed: DataFeed):
        super().__init__("bot8_sac_sortino", client, feed)
    ...
    def _build_env_from_feed(self):
        try:
            ...
            return _build_sortino_env(bars, self.selector, LOOKBACK)
    ...
    def generate_signals(self, bars_df: pd.DataFrame) -> Dict[str, float]:
        ...
        for i, sym in enumerate(self._selected):
            ...
            self.logger.log_signal(sym, signals[sym], reason=f"sortino_sac={target_w:.3f}")
        return signals
...
def _build_sortino_env(universe_bars: pd.DataFrame, selector: StockSelector, lookback: int):
    from envs.trading_env import SortinoTradingEnv
    from core.indicators import rsi, macd
    ...
    return SortinoTradingEnv(closes, ind_df, lookback=lookback)
